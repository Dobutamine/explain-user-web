...
function modelDefinitionsIndexPlugin() {
  ...
  return {
    ...
    configureServer(server) {
      ...
      // dev-only API endpoint to save a model definition to disk
      server.middlewares.use("/api/save-model-definition", (req, res) => {
        if (req.method !== "POST") {
          res.statusCode = 405;
        ...
        req.on("end", () => {
          try {
            ...
            if (!name || !data) {
              res.statusCode = 400;
              res.end(JSON.stringify({ error: "name and data are required" }));
              return;
            }
            ...
            res.end(JSON.stringify({ result: true, message: `Saved ${name}.json` }));
          } catch (e) {
            res.statusCode = 500;
...
module.exports = configure(function (/* ctx */) {
  return {
    ...
    boot: ["explain", "pixi", "bus", "model-store"],
