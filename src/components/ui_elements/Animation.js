...
export default class Animation {
  ...
  constructor(pixiApp, key, label, models, layout, xCenter, yCenter, xOffset, yOffset, radius, picto, scaling) {
    ...
    this.tinting = layout.general.tinting;

    try {
        this.ani_scale_x_prop = "." + this.layout.animation.scale_x_prop;
    } catch {
        this.ani_scale_x_prop = undefined;
    }
    
    try {
        this.ani_scale_y_prop = "." + this.layout.animation.scale_y_prop;
    } catch {
        this.ani_scale_y_prop = undefined;
    }
    ...
    try {
        this.ani_rotation_prop = "." + this.layout.animation.rot_prop;
    } catch {
        this.ani_rotation_prop = undefined;
    }
    ...
    this.sprite.scale.set(
        this.layout.sprite.scale.x  * this.global_scaling, 
        this.layout.sprite.scale.y  * this.global_scaling
    )
    ...
    this.start_pos_x = this.sprite.x;
    this.start_pos_y = this.sprite.y;
  ...
  update(data) {
    ...
    this.models.forEach((model) => {
        if (this.ani_scale_x_prop) scale_x += data[model + this.ani_scale_x_prop ]
        if (this.ani_scale_y_prop) scale_y += data[model + this.ani_scale_y_prop ]
        if (this.ani_pos_x_prop) pos_x += data[model + this.ani_pos_x_prop ]
        if (this.ani_pos_y_prop) pos_y += data[model + this.ani_pos_y_prop ]
        if (this.ani_rotation_prop) rot += data[model + this.ani_rotation_prop]
        if (this.tinting) to2 += data[model + ".to2"] / counter;
        counter += 1;
    });
    ...
    this.sprite.x = this.start_pos_x + pos_x;
    this.sprite.y = this.start_pos_y + pos_y;

    // adjust color
    if (this.tinting) {
        this.sprite.tint = this.calculateColor(to2);
    } else {
        this.sprite.tint = this.layout.sprite.color
    }
    
    // update the rotation
    this.sprite.rotation = this.layout.sprite.rotation + rot;
  }
